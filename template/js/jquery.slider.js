(function($) {
	...
	$.extend(SliderObject.prototype, {
		...
		create: function(a, b) {
			...
			//          console.log(e);
			//          console.log(e.parent().next().children('.play_main'));
			//			var bf = this.element.children("div").parent().next().children('.play_main');
			//			var zt = e.parent().next().children('.pause_main');
			...
			//          console.log(this.esqueleto.wrapper);
		...
		addEvents: function() {
			...
			var e = this.options;
			var bf = this.element.children("div").parent().parent().next().children('.play_main');
			var zt = this.element.children("div").parent().parent().next().children('.pause_main');
			//			console.log(bf);
			d.hover(function() {
			...
			bf.click(function(a) {
				...
				c.startTimer();
				bf.children('span').css('background', '#24ceed');
				zt.children('span').css('background', 'none');
			})
			//点击停止按钮轮播结束
			zt.click(function(a) {

				c.stopTimer();
				bf.children('span').css('background', 'none');
				zt.children('span').css('background', '#24ceed');
				$('.back_intro').show();
				$('.menu').show();
				$('.menu_content').show();
				$('.play_main').show();
		...
		transbar: function(b, d, e, f, g) {
			...
			$.each(bar_array, $.proxy(function(i, a) {
				...
				bar.append('<div style="position: absolute; left: -' + position + 'px; width: ' + this.options.width + '%; height: ' + this.options.height + '%;">' + e.html() + '</div>');
		...
		transsquare: function(b, c, d, e, f) {
			...
			for(iRow = 1; iRow <= this.options.rows; iRow++) {
				for(iCol = 1; iCol <= this.options.columns; iCol++) {
					...
					p.append('<div style="position: absolute; left: -' + n + 'px; top: -' + o + 'px; width: ' + this.options.width + '%; height: ' + this.options.height + '%;">' + k + '</div>');
		...
		transexplode: function(a, b, c, d, e) {
			...
			for(iRow = 1; iRow <= this.options.rows; iRow++) {
				for(iCol = 1; iCol <= this.options.columns; iCol++) {
					...
					p.append('<div style="position: absolute; left: -' + l + 'px; top: -' + m + 'px; width: ' + this.options.width + '%; height: ' + this.options.height + '%;">' + i + '</div>');
					this.element.append(p);
					var p = $('<div class="' + this.options.name + '-block ' + this.options.name + '-block-' + iCol + iRow + '" />');
					p.css({
						'overflow': 'hidden',
					...
					p.append('<div style="position: absolute; left: -' + l + 'px; top: -' + m + 'px; width: ' + this.options.width + '%; height: ' + this.options.height + '%;">' + a.html() + '</div>');
					this.element.append(p)
				}
			}
			a.css({
